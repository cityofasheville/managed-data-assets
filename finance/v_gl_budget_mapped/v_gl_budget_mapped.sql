...
glb1 AS ( -- gl_budget rows with both org & obj match for budget section
	SELECT * FROM amd.gl_budget glb
	INNER JOIN bsm ON glb.organization_id::text = bsm.org_code AND glb.object_id::text = bsm.object_code
)
SELECT 
  ...
  a.year, a.proposed_budget, a.adopted_budget,
  ...
  a.charcode_name, b.category_name, c.budget_section_name
FROM ( -- a contains all the mapped gl_budget rows
  SELECT * FROM glb1 -- gl_budget rows with both org & obj match for budget section
  UNION ALL
  ( -- This subquery pulls remaining gl_budget rows (i.e., org/obj that aren't in glb1)
    SELECT * FROM amd.gl_budget glb2
      LEFT JOIN (
        SELECT * from bsm
        WHERE bsm.object_code IS NULL
      ) AS bb ON glb2.organization_id::text = bb.org_code
    WHERE NOT ((glb2.organization_id::text, glb2.object_id::text) IN (SELECT  organization_id, object_id FROM glb1))
  )
) a
