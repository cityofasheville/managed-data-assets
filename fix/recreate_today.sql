DROP TABLE internal.coa_bc_address_master_today;

CREATE TABLE internal.coa_bc_address_master_today (
...
CREATE INDEX a23_ix1_2 ON internal.coa_bc_address_master_today USING gist (shape);
CREATE UNIQUE INDEX r31_sde_rowid_uk_2 ON internal.coa_bc_address_master_today USING btree (objectid) WITH (fillfactor='75');
CREATE INDEX search_2 ON internal.coa_bc_address_master_today USING btree (address_number, address_street_name, address_street_type, address_street_prefix, address_zipcode, address_commcode, address_unit);
----------------------------------
truncate internal.coa_bc_address_master_today;

insert into internal.coa_bc_address_master_today (
  objectid,
  civicaddress_id,
  address_x,
  address_y,
...
UPDATE internal.coa_bc_address_master_today
SET 
...
WHERE internal.coa_bc_address_master_today.location_id = water_districts.location_id;
...
WHERE internal.coa_bc_address_master_today.location_id = public_works_districts.location_id;
...
where internal.coa_bc_address_master_today.property_pin = riverupdate.pin;
...
FROM (
	SELECT 
  internal.coa_bc_address_master_today.civicaddress_id,
  internal.coa_bc_address_master_today.property_pinnum,
  CASE
  ...
  zoning_districts.zoning_links
	FROM internal.coa_bc_address_master_today
	LEFT JOIN (
	...
	ON
		internal.coa_bc_address_master_today.property_pinnum = zoning_districts.pinnum
 ) as zoningupdate
 where internal.coa_bc_address_master_today.civicaddress_id = zoningupdate.civicaddress_id;
...
FROM (
    select distinct
              ...
              n.name,
              a.civicaddress_id
            from internal.coa_bc_address_master_today as a
            LEFT OUTER JOIN internal.coa_asheville_neighborhoods n
            on (st_contains(n.shape, a.shape))
) as nbrhoods
where internal.coa_bc_address_master_today.civicaddress_id = nbrhoods.civicaddress_id;
...
where internal.coa_bc_address_master_today.civicaddress_id = h.civicaddress_id;
