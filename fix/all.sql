truncate internal.coa_bc_today;

insert into internal.coa_bc_today (
  objectid,
  civicaddress_id,
  address_x,
  address_y,
...
WHERE internal.coa_bc_today.location_id = water_districts.location_id;
...
WHERE internal.coa_bc_today.location_id = public_works_districts.location_id;
...
where internal.coa_bc_today.property_pin = riverupdate.pin;
...
FROM (
	SELECT 
  internal.coa_bc_today.civicaddress_id,
  internal.coa_bc_today.property_pinnum,
  CASE
  ...
  zoning_districts.zoning_links
	...
	ON
		internal.coa_bc_today.property_pinnum = zoning_districts.pinnum
 ) as zoningupdate
 where internal.coa_bc_today.civicaddress_id = zoningupdate.civicaddress_id;
...
FROM (
    select distinct
              ...
              n.name,
              a.civicaddress_id
            from internal.coa_bc_today as a
            LEFT OUTER JOIN internal.coa_asheville_neighborhoods n
            on (st_contains(n.shape, a.shape))
) as nbrhoods
where internal.coa_bc_today.civicaddress_id = nbrhoods.civicaddress_id;
...
where internal.coa_bc_today.civicaddress_id = h.civicaddress_id;
